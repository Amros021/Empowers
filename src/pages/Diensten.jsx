...
function LinkedInAdsAnim() {
    const containerRef = useRef(null);
    const impressiesRef = useRef(null);
    ...
    useEffect(() => {
        let ctx = gsap.context(() => {
            ...
            tl.to({ val: 0 }, {
                ...
                onUpdate: function () {
                    const ratio = this.progress();
                    if (impressiesRef.current) impressiesRef.current.innerText = Math.floor(ratio * 24800).toLocaleString('nl-NL');
                    if (leadsRef.current) leadsRef.current.innerText = Math.floor(ratio * 47);
                },
                ease: 'power1.inOut'
            })
                .fromTo(popupRef.current, { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.5, ease: 'back.out(1.7)' }, "-=1.5");
    ...
    return (
        <div ref={containerRef} className="w-full h-full bg-[#F3F2EF] rounded-3xl p-5 flex flex-col gap-3 shadow-inner overflow-hidden relative">
            ...
            <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-primary/50 uppercase tracking-widest">LinkedIn Campaign</span>
            </div>

            {/* Sponsored Post Card */}
            <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                <div className="p-3 flex items-center gap-3 border-b border-gray-100">
                    <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white font-bold text-sm shrink-0">E</div>
                    <div>
                ...
                <div className="px-3 py-2">
                    <p className="text-[10px] font-sans text-primary/80 leading-relaxed">Bereik de juiste beslissers op het juiste moment. Wij bouwen LinkedIn campagnes die B2B leads genereren. 🎯</p>
                </div>
                <div className="h-14 bg-gradient-to-r from-[#0A66C2]/10 to-primary/10 flex items-center justify-center">
            ...
            <div className="grid grid-cols-2 gap-3">
                <div className="bg-white rounded-xl p-3 border border-gray-100 shadow-sm">
                    <div className="text-[10px] font-mono text-primary/40 uppercase tracking-wider mb-1">Impressies</div>
                    <div ref={impressiesRef} className="font-mono font-bold text-primary text-lg">0</div>
                </div>
                <div className="bg-white rounded-xl p-3 border border-gray-100 shadow-sm">
                    <div className="text-[10px] font-mono text-primary/40 uppercase tracking-wider mb-1">Leads</div>
                    <div ref={leadsRef} className="font-mono font-bold text-[#0A66C2] text-lg">0</div>
                </div>
            </div>

            {/* Lead Popup */}
            <div ref={popupRef} className="absolute bottom-4 right-4 bg-[#0A66C2] text-white rounded-lg p-2 shadow-xl flex items-center gap-2 opacity-0">
                <CheckCircle2 className="w-4 h-4" />
                <span className="text-[10px] font-medium font-sans whitespace-nowrap">Lead ontvangen!</span>
            </div>
        </div>
    );
}
    ...
    return (
        <main className="min-h-screen selection:bg-accent/30 selection:text-dark flex flex-col bg-background">
            <Helmet>
                    ...
                    {JSON.stringify([
                        {
                            ...
                            "description": "Empowers beheert Google Ads campagnes voor MKB-bedrijven en webshops in Nederland. Van strategie tot optimalisatie, zodat jouw advertentiebudget maximaal rendeert.",
                            "provider": {
                                "@type": "Organization",
                                "name": "Empowers",
                        ...
                        },
                        {
                            ...
                            "description": "Empowers is gespecialiseerd in GEO: ervoor zorgen dat AI-tools zoals ChatGPT, Gemini en Claude jouw bedrijf aanbevelen. De nieuwe manier van gevonden worden naast traditionele SEO.",
                            "provider": {
                                "@type": "Organization",
                                "name": "Empowers",
                                "url": "https://www.empowers.nl"
                ...
                <meta property="og:title" content="Diensten – Meta Ads, LinkedIn Ads, TikTok Ads, GEO & SEO | Empowers" />
